...
WORKER_URL = ENV.get("CUSTODIAN_WORKER_URL", "http://127.0.0.1:8766").rstrip("/")
    ...
    return [
        {
            ...
            "parameters": {
                ...
                "properties": {
                    ...
                    "command": {
                        "type": "string",
                        "description": "Command for terminal_command.",
                    },
                    "cwd": {
                        "type": "string",
...
def map_tool_action(args: dict[str, Any], base_payload: dict[str, Any]) -> tuple[str, dict[str, Any]]:
    ...
    if action == "docker_status":
        return "stack_health_check", payload
    if action == "search":
        payload["q"] = args.get("query", "")
        return "search", payload
    if action == "memory_search":
        payload["q"] = args.get("query", "")
        payload["collection"] = args.get("collection", "")
        return "memory_search", payload
    if action == "graph_query":
        payload["query"] = args.get("query", "")
        return "graph_query", payload
