...
SYSTEM_PROMPT = """You are a magic coding assistant named Magic Coder. You specialize in generative AI tools, ComfyUI workflows, character creation, game modding, and complex software configuration. You create super cute and cool SFW images with a focus on anime-style art, wholesome character designs, and creative automation. You write code, edit configs, explain nodes, build automation, and operate on the local filesystem with enthusiasm and creativity.
...
- {"tool": "place_image_order", "character": "<character>", "scene": "<what is happening or full natural-language order>", "action": "<optional>", "pose": "<optional>", "clothing": "<optional>", "location": "<optional>", "interaction": "<optional>", "social_context": "<optional>", "mood": "<optional>", "dry_run": false} — assemble a prompt from the character profile and scene order, patch the ComfyUI workflow, and queue it to the local ComfyUI server. Free-form natural-language scene descriptions are parsed automatically. Do NOT ask the user to run external scripts and do NOT ask the user to confirm or say "queue it".
...
5. If the user places an image order (e.g. "/image Amber: ...", "draw Amber...", "render...", "scene:..."), first make sure the character is registered and the ComfyUI workflow exists. Then use `place_image_order` with dry_run=false. The prompt and patched workflow are saved to image_orders/<character>/ and the job is queued to ComfyUI immediately. Show the user the resulting positive/negative prompt, the saved order files, and the ComfyUI prompt_id. Do NOT ask the user to confirm or say "queue it".
...
def _tool_build_comfyui_workflow(
    ...
    try:
        ...
        workflow = {
            "1": {
                "inputs": {"unet_name": diffusion_model, "weight_dtype": "fp16"},
        ...
        # KSampler defaults to base lora model; optional branches are added only
        # when the caller actually supplies a reference or pose image at queue time.
...
def _tool_place_image_order(
    ...
    try:
        ...
        identity = _build_identity_prompt(profile)
        if not identity:
            return (
...
def run_magic_coder(
    ...
    elif not _current_workspace.get():
        _current_workspace.set(os.getcwd())
    ...
    try:
        ...
        return {
            "success": False,
            "text": "",
            "error": f"Magic Coder timed out after {timeout}s",
        }
    except Exception as e:
        return {"success": False, "text": "", "error": f"Magic Coder error: {e}"}
    ...
    if intent == "place_image_order":
        ...
        if not workflow_path.exists():
            ...
            if "error" in wf_result.lower():
                return wf_result
        # The user opted out of the dry-run step: image orders queue to ComfyUI
        # immediately whether they come from a slash command or natural language.
